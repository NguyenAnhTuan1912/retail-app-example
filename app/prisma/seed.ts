  ...
  await prisma.review.createMany({
    data: [
      // Điện thoại
      {
        ...
        userId: buyer1.id,
        rating: 5,
        comment: 'Máy đẹp, chạy mượt, camera chụp rất đẹp. Giao hàng nhanh!',
      },
      {
        productId: iphone15pro.id,
        userId: buyer2.id,
        rating: 4,
        comment: 'Sản phẩm tốt nhưng giá hơi cao so với thị trường.',
      },
      {
        productId: iphone15pro.id,
        userId: buyer3.id,
        rating: 5,
        comment:
          'Dùng 2 tháng rồi, rất hài lòng. Pin cải thiện nhiều so với đời trước.',
      },
      {
        productId: samsungS24.id,
        userId: buyer2.id,
        rating: 4,
        comment: 'Màn hình đẹp, S-Pen tiện lợi. Camera zoom 100x ấn tượng.',
      },
      {
        productId: samsungS24.id,
        userId: buyer3.id,
        rating: 5,
        comment: 'Galaxy AI rất hay, dịch real-time khi gọi điện quá tiện.',
      },
      {
        productId: xiaomi14.id,
        userId: buyer3.id,
        rating: 4,
        comment: 'Camera Leica chụp đẹp, giá tốt hơn iPhone nhiều.',
      },
      {
        productId: pixel8.id,
        userId: buyer1.id,
        rating: 5,
        comment:
          'Camera AI xử lý ảnh quá đỉnh, Magic Eraser xoá vật thể cực nhanh.',
      },
      {
        productId: samsungA55.id,
        userId: buyer2.id,
        rating: 4,
        comment: 'Tầm giá 9 triệu thì quá ngon, chống nước IP67 nữa.',
      },
      // Laptop
      {
        productId: macbookPro.id,
        userId: buyer1.id,
        rating: 5,
        comment: 'Hiệu năng M3 Pro quá mạnh, build chất lượng cao.',
      },
      {
        productId: macbookPro.id,
        userId: buyer3.id,
        rating: 5,
        comment: 'Compile code nhanh gấp đôi con Intel cũ, pin dùng cả ngày.',
      },
      {
        productId: dellXps.id,
        userId: buyer1.id,
        rating: 4,
        comment: 'Màn OLED rất đẹp, nhưng máy hơi nóng khi chạy nặng.',
      },
      {
        productId: rogG14.id,
        userId: buyer3.id,
        rating: 5,
        comment:
          'Chơi game mượt, màn OLED 120Hz quá đã. Quạt hơi ồn khi full load.',
      },
      {
        productId: macbookAir15.id,
        userId: buyer2.id,
        rating: 4,
        comment:
          'Màn hình lớn, không quạt nên rất yên tĩnh. Hơi nặng so với Air 13.',
      },
      // Phụ kiện
      {
        productId: airpodsPro.id,
        userId: buyer1.id,
        rating: 5,
        comment: 'Chống ồn tốt, âm thanh trong trẻo, pin trâu.',
      },
      {
        productId: airpodsPro.id,
        userId: buyer3.id,
        rating: 4,
        comment: 'ANC tốt nhưng đeo lâu hơi đau tai.',
      },
      {
        productId: sonyXm5.id,
        userId: buyer3.id,
        rating: 5,
        comment: 'Chống ồn số 1, đeo cả ngày không mỏi, âm bass sâu.',
      },
      {
        productId: sonyXm5.id,
        userId: buyer1.id,
        rating: 5,
        comment: 'Dùng để làm việc ở quán cafe, chống ồn cực kỳ hiệu quả.',
      },
      {
        productId: mxMaster.id,
        userId: buyer3.id,
        rating: 5,
        comment:
          'Chuột tốt nhất từng dùng, scroll mượt, kết nối 3 máy tiện lợi.',
      },
      {
        productId: anker.id,
        userId: buyer1.id,
        rating: 5,
        comment: 'Nhỏ gọn mà 20000mAh, sạc nhanh iPhone rất tiện.',
      },
      {
        productId: anker.id,
        userId: buyer3.id,
        rating: 4,
        comment: 'Pin trâu, sạc nhanh, nhưng hơi nặng so với mong đợi.',
      },
      // Thời trang nam
      {
        productId: polo.id,
        userId: buyer2.id,
        rating: 4,
        comment: 'Chất vải mềm, mặc thoải mái. Sẽ mua thêm màu khác.',
      },
      {
        productId: polo.id,
        userId: buyer1.id,
        rating: 5,
        comment: 'Áo đẹp, giá rẻ, giao hàng nhanh. 10 điểm!',
      },
      {
        productId: jeans.id,
        userId: buyer2.id,
        rating: 3,
        comment: 'Form hơi rộng so với size chart, nhưng chất vải ổn.',
      },
      {
        productId: bomber.id,
        userId: buyer2.id,
        rating: 4,
        comment: 'Áo nhẹ, chống nước tốt, mặc mùa thu vừa đẹp.',
      },
      {
        productId: hoodie.id,
        userId: buyer1.id,
        rating: 5,
        comment: 'Vải dày dặn, ấm, form oversize mặc rất thoải mái.',
      },
      {
        productId: sneaker.id,
        userId: buyer3.id,
        rating: 4,
        comment: 'Giày đẹp, đi êm chân, nhưng hơi trơn khi trời mưa.',
      },
      // Thời trang nữ
      {
        productId: dam.id,
        userId: buyer2.id,
        rating: 5,
        comment: 'Đầm đẹp lắm, đúng hình, chất voan mát. Rất hài lòng!',
      },
      {
        productId: somiNu.id,
        userId: buyer2.id,
        rating: 4,
        comment: 'Lụa mềm mịn, mặc đi làm rất sang. Hơi dễ nhăn.',
      },
      {
        productId: culottes.id,
        userId: buyer2.id,
        rating: 4,
        comment: 'Vải linen mát, dáng ống rộng thoải mái, cạp chun tiện.',
      },
      {
        productId: tote.id,
        userId: buyer2.id,
        rating: 3,
        comment:
          'Túi đẹp nhưng da PU hơi cứng, cần dùng một thời gian mới mềm.',
      },
    ],
  });
